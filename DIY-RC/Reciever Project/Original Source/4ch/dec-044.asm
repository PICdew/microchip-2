...
; 044-20050214 4-channel remote control decoder
;              both latched and momentary outputs are available
;
;**********************************************************************
...
rx_ok		movlw EXPIRE_TIMER ; indicate packet reception
		...
		btfsc bt, 2
		goto loop ; ch>=4: illegal with this model
		call channel_lookup
		movwf cur_ch
...
t0_int_handler	bcf INTCON, T0IF
		...
		iorwf cur_state, W
		movwf PORTA
		clrf PORTB ; all momentary on PORTB
		return
